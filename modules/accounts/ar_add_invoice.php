...
if ($_SERVER['REQUEST_METHOD']==='POST') {
	...
	if (!$error) {
		$stmt=$conn->prepare("INSERT INTO ar_invoices (customer_id, invoice_no, invoice_date, due_date, subtotal, tax_code_id, tax_amount, total) VALUES (?,?,?,?,?,?,?,?)");
		$stmt->bind_param("isssdddi", $customer_id, $invoice_no, $invoice_date, $due_date, $subtotal, $tax_code_id, $tax_amount, $total);
		if ($stmt->execute()) { $success='Invoice created.'; $_POST=[]; } else { $error='Failed: '.$conn->error; }
		$stmt->close();
	}
}
?>
	...
	<form method="POST">
		<div class="row">
			<div class="col-md-4"><div class="mb-3"><label class="form-label">Customer *</label><select name="customer_id" class="form-select" required>
				...
				<option value="<?php echo (int)$c['id']; ?>" <?php echo (($_POST['customer_id'] ?? '')==$c['id'])?'selected':''; ?>><?php echo htmlspecialchars($c['customer_name']); ?></option>
				<?php endwhile; ?>
			</select></div></div>
			<div class="col-md-4"><div class="mb-3"><label class="form-label">Invoice # *</label><input name="invoice_no" class="form-control" value="<?php echo htmlspecialchars($_POST['invoice_no'] ?? ''); ?>" required></div></div>
			<div class="col-md-2"><div class="mb-3"><label class="form-label">Date *</label><input type="date" name="invoice_date" class="form-control" value="<?php echo htmlspecialchars($_POST['invoice_date'] ?? date('Y-m-d')); ?>" required></div></div>
