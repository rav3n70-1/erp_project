...
if ($bill_id<=0) { header('Location: /erp_project/modules/accounts/accounts_payable.php'); exit(); }
$stmt=$conn->prepare("SELECT * FROM ap_bills WHERE id=?");
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
	...
	$remaining = max(0, ($bill['total'] - $bill['paid']));
	if ($amount > $remaining) { $error='Amount exceeds outstanding balance.'; }
	if (!$error) {
		$p=$conn->prepare("INSERT INTO ap_payments (bill_id, payment_date, amount, method, reference) VALUES (?,?,?,?,?)");
		$p->bind_param("isdss", $bill_id, $payment_date, $amount, $method, $reference);
		if ($p->execute()) {
			$p->close();
			// update bill paid and status
			$upd=$conn->prepare("UPDATE ap_bills SET paid = paid + ?, status = CASE WHEN paid + ? >= total THEN 'Paid' WHEN paid + ? > 0 THEN 'Partially Paid' ELSE status END WHERE id=?");
			$upd->bind_param("dddi", $amount, $amount, $amount, $bill_id);
			$upd->execute();
			$upd->close();
...
<div class="card mb-3"><div class="card-body"><div class="row"><div class="col-md-3"><strong>Bill #</strong> <?php echo htmlspecialchars($bill['bill_no']); ?></div><div class="col-md-3"><strong>Total</strong> $<?php echo number_format($bill['total'],2); ?></div><div class="col-md-3"><strong>Paid</strong> $<?php echo number_format($bill['paid'],2); ?></div><div class="col-md-3"><strong>Balance</strong> $<?php echo number_format(max(0,$bill['total']-$bill['paid']),2); ?></div></div></div></div>
